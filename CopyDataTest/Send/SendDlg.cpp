...
BEGIN_MESSAGE_MAP(CSendDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_SENDQUIT, &CSendDlg::OnBnClickedBtnSendquit)
END_MESSAGE_MAP()
...
void CSendDlg::OnBnClickedBtnSend()
{
	...
	HWND receiveWin = ::FindWindowA(NULL, "Receive");  //find the receive window

	if (receiveWin == NULL)
	{ 
		MessageBox("没找到接收窗口，发送失败!");
	}

	CString sendData;
	GetDlgItemText(IDC_EDIT_SEND,sendData);  //obtain data from send edit

	COPYDATASTRUCT copyData = {0};    //senddata copy to copydata
	copyData.lpData = sendData.GetBuffer();
	copyData.cbData = sendData.GetLength();

	::SendMessage(receiveWin, WM_COPYDATA, (WPARAM)GetSafeHwnd(), (LPARAM)&copyData);  //send the data  to receive window
	sendData.ReleaseBuffer();
}


void CSendDlg::OnBnClickedBtnSendquit()
{
	// TODO: Add your control notification handler code here
	CSendDlg::OnCancel();
}
